...
function operationFunction(num1, num2, callback) {
    ...
    const Seleccion = callback();

    if (Seleccion < 4) {
        const operacion = num1 + num2;
        console.log(`Mostrar Suma ${num1} + ${num2} = ${operacion}`);
        return operacion;

    } else if (Seleccion < 7) {
        const operacion = num1 - num2;
        console.log(`Mostrar Resta ${num1} - ${num2} = ${operacion}`);
        return operacion;

    } else {
        const operacion = num1 * num2;
        console.log(`Mostrar multiplicacion ${num1} * ${num2} = ${operacion}`);
        return operacion;
    }
}
